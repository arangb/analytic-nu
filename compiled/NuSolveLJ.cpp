...
#ifdef DEBUG_NuSolveLJ
    std::cout << endl << "L: " << endl << l << endl << "Energy: " << lep.energy() << endl << "Mass: " << Ml_ << endl;
    std::cout << endl << "B: " << endl << b << endl << "Energy: " << bjet.energy() << endl << "Mass: " << Mb_ << endl;
#endif
...
void NuSolveLJ::SetMET(double metx, double mety, double metxerr, double metyerr, double metxy)
{
    MET_(0,0) = metx;
    MET_(1,0) = mety;
    ...
    VM_(1,0) = metxy;//metxerr*metyerr*metxyrho;
    VM_(0,1) = metxy;//metxerr*metyerr*metxyrho;

    Eigen::Matrix2d VM_inv = VM_.inverse();

    //Sigm2 is the 3x3 matrix SIGMA^-2 from Burt's paper

    SIGm2_(0,0) = VM_inv(0,0);
    SIGm2_(0,1) = VM_inv(0,1);
    SIGm2_(1,0) = VM_inv(1,0);
    SIGm2_(1,1) = VM_inv(1,1);

}

PYBIND11_MODULE(pynusolver, m) {
  m.def("run_nu_solver", &run_nu_solver);
}
